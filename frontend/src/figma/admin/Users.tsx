...
export function Users() {
  ...
  useEffect(() => {
    fetchUsers().then((rows) => setUsers(rows.map(mapUser)));
  }, []);

  const handleCreate = () => {
    setSelectedUser(null);
    setFormData({ role: 'CLIENTE' });
  ...
  const handleSave = async () => {
    try {
      ...
      setUsers((current) =>
        selectedUser
          ? current.map((item) => (item.id === selectedUser.id ? mapped : item))
          : [...current, mapped],
      );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Gestion de administradores y clientes</p>
        </div>
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder={selectedUser ? 'Dejar vacio para mantener actual' : 'Contrasena'}
              />
            </div>
            <div>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta accion no se puede deshacer. El usuario "{selectedUser?.name}" sera eliminado permanentemente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
